...
#[test]
fn finalize_success_cleans_mirrored_fallback_worktree_without_cleanup_warning() -> Result<()> {
    let workspace = git_workspace()?;
    install_bundle_into_workspace(workspace.path())?;
    let runtime = Runtime::new(workspace.path())?;
    let loop_response = runtime.open_loop(open_loop_request(
        "mirrored linked cleanup success",
        "finalize-success should remove mirrored fallback worktrees without downgrading success",
    ))?;
    ...
    fs::write(
        ...
        "implemented via mirror without deleting .git\n",
    )?;
    git(&worktree_path, &["add", "feature.txt"])?;
    git(
        &worktree_path,
        &["commit", "-m", "Implement mirrored linked cleanup checkpoint"],
    )?;
    let candidate_commit_sha = git_output(&worktree_path, &["rev-parse", "HEAD"])?;
    runtime.submit_candidate_commit(SubmitCandidateCommitRequest {
        invocation_context_path: invocation_context_path(
        ...
        candidate_commit_sha: candidate_commit_sha.clone(),
        change_summary: json!({
            "headline": "Mirrored linked cleanup candidate",
            "files": ["feature.txt"],
        }),
        improvement_opportunities: None,
        notes: Some("ready for artifact review".to_owned()),
    ...
    let success_result = runtime.finalize_success(FinalizeSuccessRequest {
        ...
        integration_summary: CallerIntegrationSummary {
            ...
            resolution_notes: Some(
                "Caller-owned finalize after mirrored linked worktree cleanup".to_owned(),
            ),
        },
    })?;
...
#[test]
fn finalize_success_prunes_mirrored_registration_when_worktree_gitfile_is_missing()
-> Result<()> {
    let workspace = git_workspace()?;
    install_bundle_into_workspace(workspace.path())?;
    let runtime = Runtime::new(workspace.path())?;
    let loop_response = runtime.open_loop(open_loop_request(
        "mirrored prune success",
        "finalize-success should prune mirrored fallback registrations when the linked worktree .git file is already missing",
    ))?;
    let worktree_path = materialize_loop_worktree_with_mirrored_gitdir(
        workspace.path(),
        &loop_response.branch,
    ...
    fs::write(
        worktree_path.join("feature.txt"),
        "implemented with missing linked gitfile fallback\n",
    )?;
    git(&worktree_path, &["add", "feature.txt"])?;
    git(
        &worktree_path,
        &["commit", "-m", "Implement mirrored prune cleanup checkpoint"],
    )?;
    let candidate_commit_sha = git_output(&worktree_path, &["rev-parse", "HEAD"])?;
    runtime.submit_candidate_commit(SubmitCandidateCommitRequest {
        invocation_context_path: invocation_context_path(
        ...
        candidate_commit_sha: candidate_commit_sha.clone(),
        change_summary: json!({
            "headline": "Mirrored prune cleanup candidate",
            "files": ["feature.txt"],
        }),
        improvement_opportunities: None,
        notes: Some("ready for artifact review".to_owned()),
    ...
    fs::remove_file(worktree_path.join(".git"))?;
    fs::create_dir_all(worktree_path.join(".codex"))?;
    fs::write(
        worktree_path.join(".codex/leftover.log"),
        "leftover executor metadata after partial cleanup\n",
    )?;
    assert!(worktree_path.try_exists()?);
    assert!(mirror_path.try_exists()?);
    ...
    let success_result = runtime.finalize_success(FinalizeSuccessRequest {
        ...
        integration_summary: CallerIntegrationSummary {
            ...
            landed_commit_shas: vec![landed_head],
            resolution_notes: Some(
                "Caller-owned finalize after mirrored linked worktree lost its .git file"
                    .to_owned(),
            ),
        },
    })?;
