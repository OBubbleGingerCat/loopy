...
pub(crate) fn store_json_content(
    ...
    loopy_common_events::store_json_content(transaction, content_kind, payload)
}
...
pub(crate) fn append_event(
    ...
    loopy_common_events::append_event(transaction, loop_id, event_name, payload)
}
...
pub(crate) fn rebuild_all_projections(transaction: &Transaction<'_>) -> Result<()> {
    ...
    let records = loopy_common_projection::scan_all_event_records(transaction)?;
    loopy_common_projection::replay_event_records(records, |record| {
        replay_event_record(
            transaction,
            record.event_id,
            &record.loop_id,
...
pub(crate) fn rebuild_single_loop_projections(
    ...
    let records = loopy_common_projection::scan_loop_event_records(transaction, loop_id)?;
    loopy_common_projection::replay_event_records(records, |record| {
        replay_event_record(
            transaction,
            record.event_id,
...
pub(crate) fn append_transcript_segment(
    ...
    loopy_common_events::append_transcript_segment(
        transaction,
        invocation_id,
        speaker,
        segment_kind,
