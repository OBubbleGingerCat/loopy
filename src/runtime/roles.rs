...
pub(crate) fn load_manifest(skill_root: &Path) -> Result<Manifest> {
    let manifest_path = skill_root.join("submit-loop.toml");
...
pub(crate) fn load_task_type_role_definition(
    skill_root: &Path,
    manifest: &Manifest,
    task_type: &str,
    role_kind: &str,
...
pub(crate) fn decode_persisted_resolved_role_selection(
    _skill_root: &Path,
    value: Value,
...
pub(crate) fn normalize_open_loop_input(
    skill_root: &Path,
    manifest: &Manifest,
...
pub(crate) fn build_loopy_api_contract(allowed_terminal_apis: &[String]) -> Value {
    json!({
        "coordinator_apis": [
            "open_loop",
            "prepare_worktree",
...
pub(crate) fn build_dispatch_envelope(
...
pub(crate) fn resolve_executor_command(
...
pub(crate) fn resolve_executor_env_policy(
    executor_profile: &ExecutorProfile,
    bypass_sandbox: bool,
) -> &'static str {
...
pub(crate) fn resolve_executor_cwd(
    cwd: &str,
    workspace_root: &Path,
    worktree_path: &str,
) -> String {
...
fn resolve_template_value(
    ...
    value
        .replace(
            "{bundle_bin}",
            &skill_root.join("bin/loopy").display().to_string(),
        )
