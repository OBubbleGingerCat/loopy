...
1. Call the bundled `prepare-worktree` command for the pre-opened loop.
   ...
   If workspace-write sandboxing blocks writes under `.git/refs/heads` or `.git/worktrees`, create a fresh writable mirror under `<workspace_root>/.loopy/git-common-<label>` using separate non-destructive commands, copy `<workspace_root>/.git/.` into that mirror, and retry the fallback with an explicit `git --git-dir="<workspace_root>/.loopy/git-common-<label>" --work-tree="<workspace_root>"` worktree-add invocation before re-entering the same runtime boundary.
...
9. After any artifact approval, re-evaluate the executable checkpoint state. Do not infer that the loop is ready for handoff from a single artifact approval or accepted commit. Only call `<bundle_bin> handoff-to-caller-finalize --loop-id <loop_id> --workspace <workspace_root>` when every active checkpoint in the current executable plan is accepted. If any active checkpoint remains non-accepted, continue with the lowest-sequence remaining checkpoint instead of handing off or failing the loop.
