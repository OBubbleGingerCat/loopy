...
fn open_loop_normalizes_optional_inputs_and_persists_resolved_role_selection() -> Result<()> {
    ...
    let runtime = Runtime::with_installed_skill_root(workspace.path(), repo_root)?;
...
fn coding_task_task_type_config_uses_split_worker_defaults() -> Result<()> {
    let repo_root = Path::new(env!("CARGO_MANIFEST_DIR"));
    let task_type_text = fs::read_to_string(repo_root.join("roles/coding-task/task-type.toml"))?;
...
fn runtime_with_repo_skill(workspace_root: &Path) -> Result<Runtime> {
    Runtime::with_installed_skill_root(workspace_root, Path::new(env!("CARGO_MANIFEST_DIR")))
}
