...
fn installer_builds_and_copies_the_submit_loop_skill_bundle() -> Result<()> {
    ...
    let bundled_binary = install_root.join("bin/loopy");
    ...
    assert!(
        installed_skill.contains("./bin/loopy")
            || installed_skill.contains("bin/loopy")
            || installed_skill.contains("${SKILL_ROOT}/bin/loopy"),
    ...
    assert!(
        !installed_coordinator.contains("\"$SKILL_ROOT/bin/loopy\" finalize-success"),
...
fn installed_bundle_skill_mentions_show_loop_status_query() -> Result<()> {
    ...
    let installed_skill = fs::read_to_string(install_root.join("SKILL.md"))?;
    assert!(
        installed_skill.contains("\"$SKILL_ROOT/bin/loopy\" show-loop --loop-id <loop_id>"),
...
fn host_installed_bundle_still_uses_workspace_local_loopy_db() -> Result<()> {
    ...
    let open_loop = Command::new(install_root.join("bin/loopy"))
        .args([
            "open-loop",
            "--summary",
            "host install",
        ...
        .current_dir(workspace.path())
        .output()
        .context("failed to run host-installed bin/loopy open-loop")?;
...
fn installed_bundle_documents_exact_cli_forms_for_prompts() -> Result<()> {
    ...
    assert!(
        installed_coordinator
            .contains("\"$SKILL_ROOT/bin/loopy\" prepare-worktree --loop-id <loop_id>"),
        "installed coordinator should document the exact prepare-worktree CLI form"
    );
    assert!(
        installed_coordinator.contains(
            "\"$SKILL_ROOT/bin/loopy\" start-worker-invocation --loop-id <loop_id> --stage planning"
        ),
        "installed coordinator should document the exact planning worker CLI form"
    );
    assert!(
        installed_coordinator.contains(
            "\"$SKILL_ROOT/bin/loopy\" open-review-round --loop-id <loop_id> --review-kind checkpoint --target-type plan_revision --target-ref plan-<revision>"
        ),
        "installed coordinator should document the exact checkpoint review round CLI form"
    );
    assert!(
        installed_coordinator.contains(
            "\"$SKILL_ROOT/bin/loopy\" start-worker-invocation --loop-id <loop_id> --stage artifact --checkpoint-id <checkpoint_id>"
        ),
        "installed coordinator should document the exact artifact worker CLI form"
    );
    assert!(
        installed_coordinator.contains(
            "\"$SKILL_ROOT/bin/loopy\" open-review-round --loop-id <loop_id> --review-kind artifact --target-type checkpoint_id --target-ref <checkpoint_id>"
        ),
        "installed coordinator should document the exact artifact review round CLI form"
    );
    assert!(
        installed_coordinator.contains(
            "\"$SKILL_ROOT/bin/loopy\" start-reviewer-invocation --loop-id <loop_id> --review-round-id <review_round_id> --review-slot-id <review_slot_id>"
        ),
    ...
    assert!(
        installed_coordinator
            .contains("\"$SKILL_ROOT/bin/loopy\" handoff-to-caller-finalize --loop-id <loop_id>"),
        "installed coordinator should document the exact handoff-to-caller-finalize CLI form"
    );
    assert!(
        installed_skill
            .contains("\"$SKILL_ROOT/bin/loopy\" begin-caller-finalize --loop-id <loop_id>"),
        "installed SKILL.md should document the exact begin-caller-finalize CLI form"
    );
    assert!(
        installed_skill.contains(
            "\"$SKILL_ROOT/bin/loopy\" block-caller-finalize --loop-id <loop_id> --strategy-summary <summary> --blocking-summary <summary> --human-question <question> --conflicting-files-json <json_array>"
        ),
...
cat > "$PWD/target/$CARGO_BUILD_PROFILE/loopy" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
chmod +x "$PWD/target/$CARGO_BUILD_PROFILE/loopy"
"#,
